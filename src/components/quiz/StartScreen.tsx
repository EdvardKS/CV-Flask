...
export function StartScreen({ subject, questions, onStart, hasResume, onResume }: Props) {
  ...
  const filteredCount = useMemo(() => {
    if (cuatri === 'latest') return questions.filter(question => question.group === 'latest-test').length
    const regularQuestions = questions.filter(question => question.group !== 'latest-test')
    if (cuatri === 'all') return regularQuestions.length
    return regularQuestions.filter(question => (question.cuatrimestre ?? 1) === cuatri).length
  }, [questions, cuatri])
