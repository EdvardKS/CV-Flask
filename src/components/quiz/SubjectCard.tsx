...
export function SubjectCard({ subject }: { subject: SubjectWithCount }) {
  ...
  const aria = subject.entryMode === 'hub'
    ? `Explorar modos de estudio de ${subject.name}`
    : `Empezar test de ${subject.name}`
    ...
    >
      ...
      <span className="flex min-w-0 flex-1 flex-col">
        ...
        <span className="mt-1 flex items-center gap-2 text-xs text-slate-400">
          ...
          <span
            className="font-medium opacity-70 transition group-hover:translate-x-0.5 group-hover:opacity-100"
            style={{ color: accent }}
            aria-hidden
          >
            {cta}
